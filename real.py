...
def train_test_divider(dirtyData, feature='Pass Yards',year=2016,week=8):
    ...
    data = dirtyData
    train_data = data[(data.Year <= year) & (data.Week <= week)]

    test_data = data[(data.Year >= year) & (data.Week > week)]
...
def linearRegression(position='qb',player='Drew Brees', feature='Pass Yards'):
    ...
    averageNew = pd.read_csv('2017Data/' + 'qb' + '/' + '/actualDataWithDefense.csv')
